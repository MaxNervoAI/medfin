...
import { Plus, Building2, ChevronDown, ChevronUp, Trash2, HelpCircle, Clock, CreditCard } from 'lucide-react'
      ...
      {showFormInstitucion && (
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <Button variant="outline" onClick={() => setShowFormInstitucion(false)} size="sm">
              Cancelar
            </Button>
          </CardContent>
        </Card>
                        ...
                        <div className="bg-muted/40 rounded-lg p-3 mb-3 flex flex-col gap-3">
                          <div className="flex flex-col gap-1.5">
                            ...
                            <TipoPrestacionCombobox
                              ...
                              onChange={val => setTipoPrestacion(val)}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex flex-col gap-1.5">
                              <div className="flex items-center gap-1.5">
                                <Label htmlFor={`dias-boleta-${inst.id}`}>Plazo para emitir boleta</Label>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <HelpCircle className="size-3.5 text-muted-foreground cursor-help" />
                                  </TooltipTrigger>
                                  <TooltipContent side="top" className="max-w-52 text-center">
                                    Días máximos desde la fecha de la prestación para emitir la boleta de honorarios
                                  </TooltipContent>
                                </Tooltip>
                              </div>
                              ...
                              <p className="text-xs text-muted-foreground">días desde la prestación</p>
                            </div>
                            <div className="flex flex-col gap-1.5">
                              <div className="flex items-center gap-1.5">
                                <Label htmlFor={`dias-pago-${inst.id}`}>Plazo para recibir pago</Label>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <HelpCircle className="size-3.5 text-muted-foreground cursor-help" />
                                  </TooltipTrigger>
                                  <TooltipContent side="top" className="max-w-52 text-center">
                                    Días máximos desde la emisión de la boleta para que la institución pague el honorario
                                  </TooltipContent>
                                </Tooltip>
                              </div>
                              <div className="relative">
                                <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 size-3.5 text-muted-foreground pointer-events-none" />
                                <Input
                              ...
                              <p className="text-xs text-muted-foreground">días desde la boleta</p>
                            </div>
                          </div>
                      ...
                      {reglasInst.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-2">
                          {reglasInst.map(regla => (
                            <div key={regla.id} className="group flex items-start justify-between bg-muted/30 hover:bg-muted/50 rounded-md px-3 py-2.5 transition-colors">
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-foreground">{regla.tipo_prestacion_nombre}</p>
                                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                                    <Clock className="size-3 shrink-0" />
                                    Emitir boleta en <strong className="text-foreground">{regla.dias_emitir_boleta} días</strong> desde la prestación
                                  </span>
                                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                                    <CreditCard className="size-3 shrink-0" />
                                    Recibir pago en <strong className="text-foreground">{regla.dias_recibir_pago} días</strong> desde la boleta
                                  </span>
                                </div>
                              </div>
                              <button
                                ...
                                className="p-1.5 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100 shrink-0 ml-2"
